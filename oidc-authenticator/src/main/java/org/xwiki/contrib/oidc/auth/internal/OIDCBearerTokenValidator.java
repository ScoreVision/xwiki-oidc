...
/**
 * Validates OIDC bearer tokens from the Authorization header for REST API requests.
 *
 * @version $Id$
 * @since 2.20.4
 */
@Component(roles = OIDCBearerTokenValidator.class)
@Singleton
public class OIDCBearerTokenValidator
{
    ...
    {
        if (!this.configuration.isBearerTokenEnabled()) {
            return null;
        }
    ...
    {
        // Issuer (fail-closed)
        String expectedIssuer = this.configuration.getBearerIssuer();
    ...
    {
        String userClaim = this.configuration.getBearerUserClaim();
    ...
    private URI getJWKSURI()
    {
        ...
        String issuer = this.configuration.getBearerIssuer();
